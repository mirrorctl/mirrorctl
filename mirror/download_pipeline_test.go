...
func setupTestMirror(t *testing.T, serverURL string) (*Mirror, string) {
	...
	mirror, err := NewMirror(time.Now(), "test-mirror", config)
	if err != nil {
...
func TestDownloadBasic(t *testing.T) {
	...
	// Start download
	go mirror.download(ctx, "test/file.txt", fi, false, results)
...
func TestDownloadRetryLogic(t *testing.T) {
	...
	go mirror.download(ctx, "test/retry.txt", fi, false, results)
...
func TestDownloadContextCancellation(t *testing.T) {
	...
	// Start download
	go mirror.download(ctx, "test/slow.txt", nil, false, results)
...
func TestDownloadByHashFallback(t *testing.T) {
	...
	go mirror.download(ctx, "test/byhash.txt", fi, true, results)
...
func TestDownloadChecksumValidation(t *testing.T) {
	...
	// Start download
	go mirror.download(ctx, "test/checksum.txt", fi, false, results)
...
func TestStoreLinkBasic(t *testing.T) {
	...
	mirror, err := NewMirror(time.Now(), "test", config)
	if err != nil {
		t.Fatal("Failed to create mirror:", err)
	}
	...
	err = mirror.storeLink(fi, testFile.Name(), false)
	if err != nil {
		t.Errorf("storeLink without hash failed: %v", err)
	}
	
	// Test storeLink with hash
	err = mirror.storeLink(fi, testFile.Name(), true)
	if err != nil {
...
`
	...
	err = mirror.extractItems(indices, indexMap, itemMap, false)
	if err != nil {
...
func TestHandleResult(t *testing.T) {
	...
	processedFI, err := mirror.handleResult(result, false, false)
	if err != nil {
	...
	processedFI, err = mirror.handleResult(result404, true, false)
	if err != nil {
	...
	processedFI, err = mirror.handleResult(result404, false, false)
	if err == nil {
	...
	processedFI, err = mirror.handleResult(result500, false, false)
	if err == nil {
	...
	processedFI, err = mirror.handleResult(resultError, false, false)
	if err == nil {
...
func TestDownloadFilesIntegration(t *testing.T) {
	...
	// Test downloadFiles
	ctx := context.Background()
	downloadedFiles, err := mirror.downloadFiles(ctx, fileInfos, false, false)
	if err != nil {
...
func TestRealRepositoryDownloadPipeline(t *testing.T) {
	...
	timestamp := time.Now()
	mirror, err := NewMirror(timestamp, "slurm-pipeline-test", config)
	if err != nil {
		t.Fatal("Failed to create mirror:", err)
	}
	...
	results := make(chan *dlResult, 1)
	go mirror.download(ctx, releaseFiles[0], nil, false, results)
...
func TestDownloadPipelineErrorScenarios(t *testing.T) {
	...
	results := make(chan *dlResult, 1)
	go mirror.download(ctx, "nonexistent.txt", nil, false, results)
	result := <-results
	...
	results = make(chan *dlResult, 1)
	go mirror.download(ctx, "server-error.txt", nil, false, results)
	result = <-results
